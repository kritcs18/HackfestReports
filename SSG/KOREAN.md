...
고객은 이번 핵페스트를 통해서 챗봇의 서비스 환경을 IaaS가 아닌 Managed Service 환경으로 바꾸기를 희망했다. 그리고 가능하다면 기존의 서비스 로직을 분석해서 깔끔하게 로직을 재 정리하길 바랬으며, 동시에 서버리스 아키텍처가 적용되기를 희망했다. 고객의 생각에는 서버리스 아키텍처가 적용될 확률이 가장 높은 Layer가 바로 이 Layer일 것이라고 예상했기 때문이다. 그를 통해, Layer를 좀 더 효율적이고 효과적으로 관리할 수 있기를 원했고, 개발자들이 챗봇의 개별 로직에만 집중하면서 서비스를 지속적으로 개선해 나갈 수 있기를 바랬다.
